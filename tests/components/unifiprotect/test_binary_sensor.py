...
@pytest.mark.usefixtures("entity_registry_enabled_by_default")
async def test_binary_sensor_license_plate_detected(
    hass: HomeAssistant,
    ufp: MockUFPFixture,
    doorbell: Camera,
    unadopted_camera: Camera,
    ...
    """Test binary_sensor license plate detection entity."""

    # License plate is an opt-in smart detect type not enabled on the base
    # doorbell fixture; enabling it adds the detection-config sensor plus the
    # active-detection event binary_sensor.
    doorbell.feature_flags.smart_detect_types.append(
        SmartDetectObjectType.LICENSE_PLATE
    )

    await init_entry(hass, ufp, [doorbell, unadopted_camera])
    # Adds the active-detection event binary_sensor; the detection-config
    # sensor is NO_WRITE and filtered out for the write-capable auth user.
    assert_entity_counts(hass, Platform.BINARY_SENSOR, 16, 16)

    doorbell.smart_detect_settings.object_types.append(
        SmartDetectObjectType.LICENSE_PLATE
    )

    _, entity_id = await ids_from_device_description(
        hass, Platform.BINARY_SENSOR, doorbell, EVENT_SENSORS[7]
    )

    events = async_capture_events(hass, EVENT_STATE_CHANGED)

    # First, an active smart-detect event that does NOT yet contain the license
    # plate type: the entity must stay off.
    event = Event(
        model=ModelType.EVENT,
        id="test_event_id",
        type=EventType.SMART_DETECT,
    ...
    # Now the same event resolves with the license plate detection type and an
    # end time, so the sensor toggles on then off.
    event = Event(
        model=ModelType.EVENT,
        id="test_event_id",
        type=EventType.SMART_DETECT,
    ...
    entity_events = [
        change for change in events if change.data["entity_id"] == entity_id
    ]
    assert len(entity_events) == 3
    assert entity_events[0].data["new_state"].state == STATE_OFF
    assert entity_events[1].data["new_state"].state == STATE_ON
    assert entity_events[1].data["new_state"].attributes[ATTR_EVENT_SCORE] == 78
    assert entity_events[2].data["new_state"].state == STATE_OFF
    assert ATTR_EVENT_SCORE not in entity_events[2].data["new_state"].attributes
